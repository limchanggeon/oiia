...
| 좌석 조회 캐시·동시호출 합치기·재시도 제한 | `seats.py`: 60초 캐시(구간 단위), in-flight 병합, 동시 3, 재시도 1회, 타임아웃 20초. |
...
**코드가 아니라 설정·결정만 남은 것** — `.env`에 채우면 끝:

- 버스 실조회: `TAGO_API_KEY` (data.go.kr 활용신청)
- SRT 실조회: `SRT_ID` / `SRT_PW`
- 이메일 알림 실발송: `DEMO_EMAIL_TO` / `SMTP_*`
...
- ODsay 복합 경로 (앱 등록·IP 화이트리스트 필요)
- API.md·DB.md v3 계약 반영 (현재 v2 기준 — 팀원은 당분간 `/docs` OpenAPI 참조)
